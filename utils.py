...
def test_cleanup(logger, directory_name, files_to_del):

    try:
        del_directory(logger, directory_name)
        make_result = subprocess.run(["make", "clean"], capture_output=True, text=True, check=True)
        #print(f"Make clean: stdout:\n{make_result.stdout}")
        ...
        for file in files_to_del:
            ...
            if os.path.exists(path):
                os.remove(path)
                print_and_log(logger, f"Removed file: {path}")

    except Exception as e:
        print_and_log_error(logger, f"Test cleanup failed. Please do it manually: {e}")
