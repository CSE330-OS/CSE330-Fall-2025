...
parser.add_argument('--devX', type=str, required=True, help='Source Device', default='/dev/devX')
parser.add_argument('--devY', type=str, required=True, help='Cache Device', default='/dev/devY')
...
print_and_log(logger, f"Source Device: /dev/{devX}")
print_and_log(logger, f"Cache Device: /dev/{devY}")
print_and_log(logger, "++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++")
...
for index, row in roster_df.iterrows():
    ...
    if zip_files and os.path.isfile(zip_files[0]):
        ...
        km_c        = check_file_exists(logger, "extracted/source_code/dm_lru", [".c"])
        ...
        if sanity_pass:
            ...
            cse330_grader       = grader_project5(logger, asuid)

            source_code_path    = find_source_code_path(extracted_folder)
            dmcache_ko          = f"{source_code_path}/dmcache.ko"

            if not os.path.exists(source_code_path):
                raise FileNotFoundError(f"Source code path does not exist: {source_code_path}")

            dm_lru_path = os.path.join(source_code_path, "dm_lru.c")
            if not os.path.exists(dm_lru_path):
                raise FileNotFoundError(f"dm_lru.c not found in {source_code_path}")

            print(f"Found dm_lru.c  at {dm_lru_path}")
            current_directory = os.getcwd()
            shutil.copy(dm_lru_path, current_directory)
            print(f"Copied 'dm_lru.c' to {current_directory}")
            ...
            if make_pass:
                ...
                for num, hit_ratio in test_cases:
                    ...
                    test_folder = current_directory 

                    restart_required, result    = cse330_grader.execute_kernel_module(asuid, test_folder, test_module_script, num, devX, devY, hit_ratio)
                ...
                #del_directory(logger, extracted_folder)
                test_cleanup(logger, extracted_folder, ["dm_lru.c"])
